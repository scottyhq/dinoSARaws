...
# process 3 interferograms (main=20170927 with 3 preceding dates)
# NOTE: this will run topsApp.py for subswath 2, and region of interest defined by search
proc_batch_main.py -p 115 -m 20170927 -s 3 -n 2 -r 44.0 44.5 -122.0 -121.5 -g 44.0 44.5 -122.0 -121.5
...
def cmdLineParse():
    ...
    parser.add_argument('-m', type=str, dest='main', required=True,
            help='Main date')
    parser.add_argument('-s', type=int, dest='subordinate', required=True,
            help='Number of subordinates')
...
def get_pairs(inps):
    ''' 
    Load geojson inventory, find preceding pairs to interfere with main date
    '''
...
def create_s3buckets(inps,subordinateList):
    '''
    Create S3 buckets for given main
    '''

    s3 = boto3.resource('s3')
    for subordinate in subordinateList:
        bucketName = 'int-{0}-{1}'.format(inps.main, subordinate)
        s3.create_bucket(bucketName)
        bashFile = write_bash_script(inps.main, subordinate)
        s3.upload_file(bashFile, bucketName, bashFile)


def write_bash_script(inps, subordinate)
    filename = 'proc-{main}-{subordinate}.sh'.format(**vars(inps))
