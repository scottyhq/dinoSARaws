...
if __name__ == '__main__':
    ...
    main_urls  = get_slc_urls(gf, inps.main, inps.path)
    subordinate_urls  = get_slc_urls(gf, inps.subordinate, inps.path)
    downloadList = main_urls + subordinate_urls
    inps.main_scenes = [os.path.basename(x) for x in main_urls]
    inps.subordinate_scenes = [os.path.basename(x) for x in subordinate_urls]

    if inps.poeorb:
        try:
            frame = os.path.basename(inps.main_scenes[0])
            downloadList.append(get_orbit_url(frame))
            frame = os.path.basename(inps.subordinate_scenes[0])
            downloadList.append(get_orbit_url(frame))
        except Exception as e:
